...
These skills are Markdown, with two exceptions: `traigent-setup-audit` ships local scripts (standard library only; nothing leaves the machine unless an `--approve` names the check) and `traigent-analyze-variable-importance` ships a local ranking script (optionally imports the SDK, no network). Neither reads credentials; the rest make no calls at all.
