...
| Optimize | [traigent-optimize-run](skills/traigent-optimize-run/) | Run optimization end-to-end — async/sync execution via `func.optimize()` and `optimize_sync()`, algorithm selection (auto/grid/random anywhere; named smart selectors like bayesian/optuna on authenticated connected runs, SDK 0.20.1+), trial limits, cost budgets, `ParallelConfig` for concurrent trials, and `CostLimitExceeded` handling. |
| Analyze | [traigent-analyze-guidance](skills/traigent-analyze-guidance/) | What should this run be, and what next? Three modes in one skill: (A) pre-run — fetch the service run-plan, apply preflight, mock dry-run first, and launch only on explicit go; (B) post-run, portal-tracked — fetch `traigent guidance next RUN_ID --json`, validate the precommitted Planner V2 treatment/profile and opaque authoritative decision, then resolve only its static decision command; (C) offline/local fallback — diagnose flat/noisy/negative results when no service payload exists. Portal-tracked decisions come from Traigent, never local markdown logic. |
