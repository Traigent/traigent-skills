...
The eight skills that propose, size, or judge a run also carry a shared **economics and characterization reference**: the Traigent posture on spending, the five closed characterization questions, how the agent should ask them, and the mandatory duty to explain every recommendation in the user's own numbers. It is authored once in [`docs/shared/economics-characterization.v0.md`](docs/shared/economics-characterization.v0.md) and generated byte-identically into each of those skills as `references/economics-characterization.v0.md` by `python tools/contract/sync_economics_reference.py`, so the reference travels with a single-skill install (`npx skills add --skill <one>`) rather than dangling at a repo path you never cloned. The per-skill copies are generated artifacts — edit the canonical file, re-run the sync tool, then `python tools/contract/update_reference_hashes.py`.
...
| Evaluation | [traigent-eval-audit](skills/traigent-eval-audit/) | Audit evaluator and LLM-judge reliability — task-fit calibration of any scorer on good / equivalent / partial / bad answers, gold-slice agreement, repetition stability, bias probes (position, length, self-preference), parse-failure policy, threshold calibration, and when to demote a judge to a hybrid gate. |
...
| Analyze | [traigent-analyze-guidance](skills/traigent-analyze-guidance/) | What should this run be, and what next? Three modes in one skill: (A) pre-run — fetch the service run-plan, apply preflight, mock dry-run first, and launch only on explicit go; (B) post-run, portal-tracked — fetch the backend decision brief via the `analytics_get_run_decision_brief` MCP tool and present its headline, confidence, recommended action, evidence, and warnings verbatim (the retired Planner V2 guidance CLI has no current SDK replacement for its execute/receipt verbs); (C) offline/local fallback — diagnose flat/noisy/negative results when no service payload exists. Portal-tracked decisions come from Traigent, never local markdown logic. |
| Analyze | [traigent-analyze-results](skills/traigent-analyze-results/) | Inspect `OptimizationResult` objects — best config and score, individual trial comparison, stop-reason interpretation, the SDK's `best_config_margin` winner verdict and free `eval_audit`, cost/token usage tracking, and `export_config()` / `apply_best_config()` — apply is a promotion step gated by `traigent-ci-safety-gate`. |
...
| Gate & Debug | [traigent-debugging](skills/traigent-debugging/) | Troubleshoot optimization failures — mock mode for CI/CD without API keys, `TRAIGENT_LOG_LEVEL=DEBUG` logging, error-class reference (`CostLimitExceeded`, `ConfigurationError`, `OptimizationStateError`), portal / local-fallback diagnosis for a connected run that ended local-only, and missing-dependency diagnosis. |
...
These skills are Markdown, with one exception: `traigent-setup-audit` ships local scripts, and the skill states their network behaviour — nothing leaves the machine unless an `--approve` names the check. The rest make no calls and read no credentials.

## Requirements

**To use the skills themselves: nothing.** They're Markdown instructions your AI coding agent reads — no runtime, nothing to install (the `traigent-setup-audit` scripts run on any Python 3.11+ interpreter already present).
