...
| `score-relocation` | `trial.score` / `metrics["score"]` | Single built-in objective: since 0.22.0 (the first release after 0.21.3), `Trial.score` and `metrics["score"]` carry the primary objective's value; when a custom `scoring_function` owns "accuracy", the builtin exact-match value is recorded as `metrics["exact_match_default"]` (on 0.21.3 and earlier it appeared as `metrics["score"]`). Multi-objective (weighted) runs: both carry the weighted, run-range-normalised selection basis, not any single objective (since 0.22.0 for every multi-objective run, including default equal weights; on 0.19.1-0.21.3 only non-uniform weights rewrote `metrics["score"]`, and `trial.score` does not exist before 0.22.0); `results.best_score` still reports the primary objective. Custom `metric_functions` primary objective: `trial.score` is the objective, `metrics["score"]` is the builtin exact-match value. Read objectives by name: `trial.metrics["<objective>"]`. | 0.22.0 | Traigent/Traigent#1845, #1849 | `score` mirrors a single built-in primary objective on SDKs after 0.21.3; read objectives by name |
...
| `backend-url` | `traigent plan` / retired `traigent next-steps` URL resolution | Since 0.20.0 these commands resolve the backend URL as flag → `TRAIGENT_BACKEND_URL` → the URL stored by `traigent auth login` → the local default. On 0.19.x and earlier they ignored the stored auth-login URL and always defaulted to localhost unless the flag or env var was passed. `next-steps` was later retired from the SDK (2026-08-03, absent from 0.26.0), so on current SDKs this row applies to `traigent plan` only. | 0.20.0 | Traigent/Traigent#1721 | `plan` honors the stored auth-login URL since 0.20.0 |
