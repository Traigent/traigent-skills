...
| `score-relocation` | `trial.score` / `metrics["score"]` | Single built-in objective: since 0.22.0 (the first release after 0.21.3), `Trial.score` and `metrics["score"]` carry the primary objective's value; when a custom `scoring_function` owns "accuracy", the builtin exact-match value is recorded as `metrics["exact_match_default"]` (on 0.21.3 and earlier it appeared as `metrics["score"]`). Multi-objective (weighted) runs: both carry the weighted, run-range-normalised selection basis, not any single objective (since 0.19.1); `results.best_score` still reports the primary objective. Custom `metric_functions` primary objective: `trial.score` is the objective, `metrics["score"]` is the builtin exact-match value. Read objectives by name: `trial.metrics["<objective>"]`. | 0.22.0 | Traigent/Traigent#1845, #1849 | `score` mirrors a single built-in primary objective on SDKs after 0.21.3; read objectives by name |
