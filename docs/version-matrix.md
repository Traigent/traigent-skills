...
| `unmeasured-cost-cap` | `TRAIGENT_FALLBACK_TRIAL_LIMIT`, `COST_UNMEASURED_*` warnings | Since 0.30.0, a run whose cost cannot be measured stops after a default safety limit of 10 trials (`TRAIGENT_FALLBACK_TRIAL_LIMIT` changes it) with `stop_reason="cost_limit"` and warning `COST_UNMEASURED_TRIAL_LIMIT_REACHED`, unless `max_trials` or `max_total_examples` was set explicitly — that counts as consent, and the run goes up to that size with warning `COST_UNMEASURED_TRIALS_RAN` instead (`max_total_examples` alone does not raise the default `max_trials` of 10). Raw OpenAI SDK calls (`openai.OpenAI`/`openai.AsyncOpenAI`) are cost-captured for the first time, non-streaming only, when the OpenAI override is active. Before 0.30.0 an unmeasured cost-objective trial could report `$0` and win a cost objective, and raw OpenAI calls were never captured. | 0.30.0 | Traigent/Traigent#2441, #2447 | unmeasured cost stops the run and is reported as unknown, not $0, since 0.30.0 |
