...
<!-- Skills POINT here (one line + a role sentence). Never copy the posture or the question -->
<!-- set into a SKILL.md — duplication is what this file exists to prevent. -->
...
**Reconciling with the interaction policy.** The shared interaction policy caps presented
options at three with exactly one marked **Recommended**. The closed fields here have five
values. Resolve it this way: present the recommended value plus the nearest one or two
plausible alternatives (three shown, one recommended), and offer the full list on request.
The *submitted* value must still be one of the five closed values — presentation narrows, the
contract does not.
