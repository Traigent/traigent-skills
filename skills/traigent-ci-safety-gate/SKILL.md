...
leaves under `traigent-runs/`) — that is held-back and non-blind, and a statistical gate reads ten
rows as `no_decision` at any `min_effect` worth setting; build a sealed slice sized for the effect you require. The gate judges
