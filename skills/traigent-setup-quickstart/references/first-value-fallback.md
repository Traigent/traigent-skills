...
straight to `traigent-dataset-curate` (turn their examples into the canonical dataset
contract) and `traigent-eval-build` (score real output) — don't make them run the
...
- **Real data** — `traigent-dataset-curate` for the canonical dataset contract, then
  `traigent-eval-build` to score actual output against it.
- **A real decorator** — `traigent-setup-decorator` for evaluators, injection mode,
  execution policy, and weighted objectives.
- **After a run** — `traigent-analyze-guidance` for what to change on the next iteration.
