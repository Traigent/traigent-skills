...
| [traigent-knob-iteration](traigent-knob-iteration/SKILL.md) | Measure which knobs moved accuracy, drop the dead ones, swap in better structural knobs (the swap that went 83.3% -> 90%). |
| [traigent-results-consolidation](traigent-results-consolidation/SKILL.md) | Consolidate all runs over one domain into a single sortable master workbook (one row per eval, incl. permutation count per run). |
