...
|---|---|---|---|
| `algorithm` | `str` | `"auto"` | `"auto"` uses the Traigent **cloud smart optimizer** while your trials run in your environment. `"grid"`/`"random"` run local search in the SDK. Named smart selectors: on SDK >= 0.20.1, `"bayesian"`/`"tpe"`/`"optuna"`/`"optuna_tpe"`/`"optuna_random"` bind to the typed backend Optuna strategy in authenticated connected runs, while `"optuna_grid"`, `"optuna_cmaes"`, `"optuna_nsga2"`, `"nsga2"`, `"cmaes"`, `"nsgaii"`, `"nsga_ii"`, `"cma_es"` fail fast with a capability message (SDK CHANGELOG `[0.20.1]`; Traigent/Traigent#1752/#1758). None of them run locally or offline: `offline=True` raises `ConfigurationError`, the local registry raises `OptimizationError` (live-verified on 0.21.0). Unknown names are rejected. |
...
- **Named smart selectors — connected-only; supported names bind since SDK 0.20.1.** In authenticated connected runs, `"bayesian"`/`"tpe"`/`"optuna"`/`"optuna_tpe"`/`"optuna_random"` bind to the typed backend Optuna strategy (SDK CHANGELOG `[0.20.1]`; Traigent/Traigent#1752/#1758 — on 0.20.0 no smart name executed end-to-end); unsupported family aliases fail fast with a capability message. They never run locally: `ConfigurationError` at decoration time with `offline=True`, `OptimizationError` from the local registry (live-verified on 0.21.0). Use `"auto"` for zero-config connected smart optimization and `"grid"`/`"random"` only for explicit local/offline search.
