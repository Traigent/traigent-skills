...
> **⚠️ Reasoning models need more than the default `max_tokens` range.** `IntRange.max_tokens()` defaults to `[256, 1024]`, sized for non-reasoning models. Reasoning models (`gemini-2.5`/`3.x`, `gpt-5`, the `o`-series) spend hidden reasoning tokens that count against `max_tokens` *before* any answer text, so a value from the low end of that range can be fully consumed by reasoning and truncate the answer mid-output (`finish_reason=length`) — making the capable model score *far below* a cheap non-reasoning one as a measurement artifact. When the sweep includes reasoning models, use `IntRange.max_tokens(task="long")` (`[1024, 4096]`) or otherwise give **≥1024–2048** of headroom. The floor is *effort-dependent*: under an aggressive native effort setting (`reasoning_effort="high"`) truncation has been observed even at `4096`, so raise the cap further — **≥8192** when `high` is in the space (`task="long"`'s `[1024, 4096]` upper bound is sized for default/low-effort reasoning, not `high`). Field-observed: `gemini-2.5-pro` at `max_tokens=256` spent 241 tokens reasoning and emitted a truncated query (~23% of the expected output); at `1536` it completed correctly. This is the canonical `max_tokens` headroom floor for reasoning models — the `traigent-setup-integrations` litellm reference and this skill's reasoning-effort section both defer here.
...
| `Choices.model(provider="openai", tier="quality")`| `Choices[str]` | gpt-4o, gpt-5.2 (0.25.0+; earlier builds listed the retired o1-preview) |
