...
- A variable is called `significant` only when its per-value spread beats a no-effect baseline at the configured confidence. The max−min spread is non-negative by construction, so a bootstrap CI clearing 0 is not a significance test — the CI is shown for scale only.
