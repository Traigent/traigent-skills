...
def test_unpublished_js_sdk_is_not_taught_as_public_npm_install(repo_root: Path) -> None:
    offenders: list[str] = []
    for path in _markdown_files(repo_root):
        for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            lowered = line.lower()
            if UNPUBLISHED_NPM_INSTALL_RE.search(line) and "not published" not in lowered and "404" not in lowered:
