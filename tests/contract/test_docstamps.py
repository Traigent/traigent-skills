...
def test_extractor_ignores_malformed_stamp_body(tmp_path: Path) -> None:
    path = tmp_path / "SKILL.md"
    path.write_text(
        "<!-- contract: this is not a recognized stamp grammar -->\n",
        encoding="utf-8",
    )
...
def test_docstamp_asserter_rejects_wrong_path_fixture(tmp_path: Path) -> None:
    """Same red-before-green proof for the `path` check (raises is exercised
    live by the real `ConfigurationError` stamp in traigent-optimize-run)."""
    path = tmp_path / "SKILL.md"
    path.write_text(
