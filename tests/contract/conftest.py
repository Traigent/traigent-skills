...
def _docstamp_fact_in_bucket(fact: ContractFact, config: pytest.Config) -> bool:
    """Per-stamp version floor from the optional ``@ SDK <ver>`` suffix.

    Mirrors ``_env_fact_in_bucket``: a stamp restamped against a newer SDK than
    the skill's own floor is only checked in buckets at or above its own
    ``@ SDK`` marker, so an older bucket does not red a claim that is honestly
