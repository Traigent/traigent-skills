...
class ObfuscationVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Call):
                ...
                if (
                    ...
                    and inner_call.func.id == "__import__"
                ):
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            attr_name = node.func.attr
            if obj_name in ("os", "subprocess") and attr_name in (
                "system",
                "popen",
                "call",
                "run",
        ...
        if (
            isinstance(node.func, ast.Attribute)
            and node.func.attr in ("FunctionType", "CodeType")
            and isinstance(node.func.value, ast.Name)
            ...
            if isinstance(val, ast.Call):
                if isinstance(val.func, ast.Name) and val.func.id in (
                    ...
                    if val.args and isinstance(val.args[0], ast.List):
                        if val.args[0].elts and isinstance(
                            val.args[0].elts[0], ast.Constant
                        ):
                            self._add(
                                severity="high",
