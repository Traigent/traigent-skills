...
def scan_dockerfile(file_path, rel_path):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if (
                re.search(r"(?i)(password|secret|token|key)\s*=", stripped)
                and "ENV" in stripped
            ):
                findings.append(
                    core.Finding(
        ...
        if not has_user:
            findings.append(
                core.Finding(
                    ...
                    file=rel_path,
                    line=0,
                    snippet="Missing USER directive",
                    category="container-config",
                )
            )
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if m:
                ...
                is_official = action.startswith("actions/") or action.startswith(
                    "github/"
                )
            ...
            if "${{ secrets." in stripped and (
                "run:" in stripped or "run: |" in stripped
            ):
                findings.append(
                    core.Finding(
                        scanner=SCANNER_NAME,
            ...
            if "${{ github.event." in stripped and (
                "run:" in stripped or "run: |" in stripped
            ):
                findings.append(
                    core.Finding(
...
def scan_claude_config(file_path, rel_path):
    ...
    try:
        ...
        hooks_patterns = [
            (
                re.compile(r'(?i)"hooks"\s*:'),
                "hooks section present in Claude Code settings",
            ),
            (
                re.compile(
                    r'(?i)"(PreToolUse|PostToolUse|UserPromptSubmit|Stop|SessionStart|SessionEnd)"\s*:'
                ),
                "Claude Code hook event handler",
            ),
            (
                re.compile(
                    r'(?i)"command"\s*:\s*"[^"]{0,300}(curl|wget|bash|sh|python|node|exec|eval|base64)[^"]{0,300}"'
                ),
                "Shell/download command in Claude Code hook (CVE-2025-59536 RCE vector)",
            ),
        ]
            ...
            if ".github/workflows" in file_path:
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        yaml_content = f.read()
                    if any(
                        marker in yaml_content
                        for marker in ("apiVersion:", "kind:", "metadata:")
                    ):
