...
DYNAMIC_IMPORT_PATTERNS = [
    ...
    (
        re.compile(
            r"__import__\s*\(\s*(os\.environ|os\.getenv|config|settings|variable|getattr|input)"
        ),
        "__import__() with dynamic source",
    ),
    (
    ...
    (
        ...
        "__import__() call (dynamic import evasion)",
    ),
    # JavaScript/TypeScript dynamic imports
    (
        re.compile(r"require\s*\(\s*[a-zA-Z_]\w*\s*\)"),
        "require() with variable argument (JS)",
    ),
    (
        re.compile(r"import\s*\(\s*[a-zA-Z_]\w*\s*\)"),
        "dynamic import() with variable argument (ES)",
    ),
    (
        re.compile(r"require\s*\(\s*(process\.env|config|options)"),
        "require() from config/env (JS)",
    ),
    (
        re.compile(r"import\s*\(\s*(process\.env|config|options)"),
        "dynamic import() from config/env (ES)",
    ),
]
...
FETCH_EXECUTE_PATTERNS = [
    ...
    (
        re.compile(r"eval\s*\(\s*(requests\.(get|post)|urllib)"),
        "eval() of HTTP response content",
    ),
    (
        re.compile(r"exec\s*\(\s*(requests\.(get|post)|urllib)"),
        "exec() of HTTP response content",
    ),
    (
        re.compile(
            r"(requests\.(get|post)|urllib\w*\.urlopen)\s*\(.*\)\s*\.\s*(text|read|content|decode).*\beval\b"
        ),
        "HTTP fetch piped to eval()",
    ),
    (
        re.compile(
            r"(requests\.(get|post)|urllib\w*\.urlopen)\s*\(.*\)\s*\.\s*(text|read|content|decode).*\bexec\b"
        ),
        "HTTP fetch piped to exec()",
    ),
    # NOTE: Runtime pip/npm install patterns are owned by scan_manifest_drift.py
    # Download then run shell scripts
    (
        re.compile(
            r"(requests\.get|urllib\w*\.urlopen)\s*\(.*\).*\.(write|save).*\.(sh|py|ps1|bat)"
        ),
        "Download and save executable script",
    ),
    (
        re.compile(r'subprocess\.\w+\s*\(\s*\[?\s*["\']curl["\'].*\|\s*["\']?bash'),
        "curl piped to bash via subprocess",
    ),
    # Two-line patterns: download to variable, then exec/eval the variable
    (
        re.compile(r"=\s*(requests\.(get|post)|urllib\w*\.urlopen)\s*\(.*https?://"),
        "HTTP response stored to variable (check for subsequent exec)",
    ),
    # JavaScript: fetch -> eval
    (re.compile(r"eval\s*\(\s*await\s+fetch"), "eval() of fetched content (JS)"),
    (
        re.compile(r"new\s+Function\s*\(\s*await\s+fetch"),
        "new Function() from fetched content (JS)",
    ),
]
...
SELF_MOD_PATTERNS = [
    ...
    (
        re.compile(r"SourcelessFileLoader"),
        "SourcelessFileLoader - bytecode loading bypass (CVE-2026-2297)",
    ),
    (
        re.compile(r"spec_from_file_location\s*\(.*\.pyc"),
        "spec_from_file_location loading .pyc (CVE-2026-2297 variant)",
    ),
    (
        re.compile(r"module_from_spec\s*\("),
        "module_from_spec() - runtime module loading from spec",
    ),
    (
        re.compile(r'compile\s*\(\s*[a-zA-Z_]\w*\s*,\s*["\']'),
        "compile() with variable source - runtime code generation",
    ),
    # JavaScript self-modification (scan_ast.py is Python-only)
    (
        re.compile(r"new\s+Function\s*\(\s*[a-zA-Z_]"),
        "new Function() with variable body (JS)",
    ),
    (
        re.compile(r"eval\s*\(\s*atob\s*\("),
        "eval(atob()) - base64 decode and execute (JS)",
    ),
]
...
TIME_BOMB_PATTERNS = [
    ...
    (
        re.compile(r"datetime\.\w*\(\s*\d{4}\s*,"),
        "Hardcoded future datetime comparison (potential time bomb)",
    ),
    (
        re.compile(r"datetime\.now\(\)\s*[><=]+"),
        "datetime.now() comparison (potential activation trigger)",
    ),
    (
        re.compile(r"date\.today\(\)\s*[><=]+"),
        "date.today() comparison (potential activation trigger)",
    ),
    # Unix timestamp comparisons
    (
        re.compile(r"time\.time\(\)\s*[><=]+\s*\d{9,}"),
        "time.time() compared to hardcoded unix timestamp",
    ),
    (
        re.compile(r"time\.time\(\)\s*[><=]+\s*\w"),
        "time.time() compared to variable (potential time bomb)",
    ),
    (
        re.compile(r"int\(time\.time\(\)\)\s*[><=]+\s*\d{9,}"),
        "Unix timestamp comparison (potential time bomb)",
    ),
    # Counter/attempt-based activation
    (
        re.compile(
            r"(?:count|counter|attempts?|calls?|invocations?)\s*[><=]+\s*\d{2,}"
        ),
        "Counter-based activation threshold",
    ),
    # Random/probabilistic triggers
    (
        re.compile(
            r"random\.\w+\(\)\s*[<>=]+\s*0\.\d+.*(?:exec|eval|system|subprocess|import)"
        ),
        "Probabilistic trigger with code execution",
    ),
    # JavaScript time comparisons
    (
        re.compile(r"Date\.now\(\)\s*[><=]+\s*\d{12,}"),
        "Date.now() compared to hardcoded timestamp (JS)",
    ),
    (
        re.compile(r'new\s+Date\(\)\s*[><=]+\s*new\s+Date\s*\(\s*["\']'),
        "Date comparison with hardcoded date (JS)",
    ),
]
...
class RuntimeDynamismASTVisitor(ast.NodeVisitor):
    ...
    def visit_Compare(self, node):
        ...
        for comparator in node.comparators:
            if self._is_datetime_call(left) and self._is_datetime_constructor(
                comparator
            ):
                self._add(
                    "high",
            ...
            elif self._is_datetime_call(comparator) and self._is_datetime_constructor(
                left
            ):
                self._add(
                    "high",
...
def scan_file_regex(file_path, rel_path):
    ...
    try:
        ...
        print(f"[!] {SCANNER_NAME}: Failed to read {rel_path}: {e}", file=sys.stderr)
        return []
    ...
    findings.extend(
        core.scan_patterns(
            content, rel_path, TIME_BOMB_PATTERNS, "time-bomb", "medium", SCANNER_NAME
        )
    )
...
def scan_file_ast(file_path, rel_path):
    ...
    try:
        ...
        print(f"[!] {SCANNER_NAME}: Failed to parse {rel_path}: {e}", file=sys.stderr)
        return []
