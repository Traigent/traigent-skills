...
RUNTIME_INSTALL_PATTERNS = [
    (
        re.compile(r'subprocess\.\w+\s*\(\s*\[?\s*["\']pip["\'],?\s*["\']install["\']'),
        "Runtime pip install via subprocess",
    ),
    (
        re.compile(r'os\.system\s*\(\s*["\']pip\s+install'),
        "Runtime pip install via os.system()",
    ),
    (
        re.compile(r'os\.system\s*\(\s*["\']pip3\s+install'),
        "Runtime pip3 install via os.system()",
    ),
    (
        re.compile(r'subprocess\.\w+\s*\(\s*\[?\s*["\']npm["\'],?\s*["\']install["\']'),
        "Runtime npm install via subprocess",
    ),
    (
        re.compile(r'os\.system\s*\(\s*["\']npm\s+install'),
        "Runtime npm install via os.system()",
    ),
    (
        re.compile(r'subprocess\.\w+\s*\(\s*["\']pip\s+install'),
        "Runtime pip install via subprocess string",
    ),
    (
        re.compile(r'check_call\s*\(\s*\[.*["\']pip["\'].*["\']install["\']'),
        "Runtime pip install via check_call",
    ),
    (
        re.compile(
            r'check_call\s*\(\s*\[.*sys\.executable.*["\']-m["\'].*["\']pip["\']'
        ),
        "Runtime pip install via sys.executable",
    ),
]
...
def parse_python_requirements(repo_path):
    ...
    for req_name in req_files:
        ...
        if os.path.exists(req_path):
            try:
                ...
                print(
                    f"[!] {SCANNER_NAME}: Failed to parse {req_name}: {e}",
                    file=sys.stderr,
                )
    ...
    if os.path.exists(pyproject_path):
        try:
            ...
            print(
                f"[!] {SCANNER_NAME}: Failed to parse pyproject.toml: {e}",
                file=sys.stderr,
            )
    ...
    if os.path.exists(setup_path):
        try:
            ...
            print(f"[!] {SCANNER_NAME}: Failed to parse setup.py: {e}", file=sys.stderr)
...
def parse_node_dependencies(repo_path):
    ...
    if os.path.exists(pkg_path):
        try:
            ...
            print(
                f"[!] {SCANNER_NAME}: Failed to parse package.json: {e}",
                file=sys.stderr,
            )
...
def extract_python_imports(file_path):
    ...
    try:
        ...
        print(
            f"[!] {SCANNER_NAME}: Failed to extract imports from {file_path}: {e}",
            file=sys.stderr,
        )
        return set()
...
def extract_js_imports(file_path):
    ...
    try:
        ...
        print(f"[!] {SCANNER_NAME}: Failed to read {file_path}: {e}", file=sys.stderr)
        return set()
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Try):
            ...
            for handler in node.handlers:
                ...
                if re.search(
                    r"pip\s+install|subprocess|os\.system|check_call.*install",
                    handler_src,
                ):
                    lineno = getattr(node, "lineno", 0)
                    snippet = (
                        source_lines[lineno - 1].strip()[:120]
...
def scan_manifest_drift(repo_path):
    ...
    has_js_manifest = bool(js_declared) or os.path.exists(
...
def scan_runtime_installs(repo_path):
    ...
    for file_path, rel_path in core.walk_repo(
        ...
        try:
            ...
            print(
                f"[!] {SCANNER_NAME}: Failed to read {rel_path}: {e}", file=sys.stderr
            )
            continue
