...
INSTALL_PATTERNS = [
    ...
    (
        re.compile(
            r"git\s+clone\s+(?:--[^\s]+\s+)*(?:https?://|git@)([^\s]+)(?:\s+([^\s]+))?"
        ),
        "git_clone",
    ),
...
def extract_package_names(pattern_type, match):
    ...
    if pattern_type in (
        ...
        names = [
            n.strip() for n in cleaned.split() if n.strip() and not n.startswith("-")
        ]
...
def _is_safe_scan_path(resolved_path):
    ...
    return (
        ...
        or resolved_path.startswith("/tmp/")
        or resolved_path.startswith("/private/tmp/")
    )
...
def run_scanner(scanner_script, repo_path):
    ...
    try:
        ...
        print(f"[!] auto_scan: Scanner {scanner_script} failed: {e}", file=sys.stderr)
    return []
...
def run_targeted_scan(repo_path):
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        futures = {
            executor.submit(run_scanner, s, repo_path): s for s in targeted_scanners
        }
...
def format_output(findings, command="", pattern_type=""):
    ...
    if len(findings) > 15:
        lines.append(
            f"... and {len(findings) - 15} more findings. Run full scan for details."
        )

    if critical_count > 0:
        lines.append(
            f"**VERDICT: {critical_count} CRITICAL finding(s). Do not proceed without review.**"
        )
    elif high_count > 0:
        lines.append(
            f"**VERDICT: {high_count} HIGH finding(s). Review before proceeding.**"
        )
