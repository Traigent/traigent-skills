    ...
    try:
        ...
        if "hooks" in data:
            ...
            for event_name, hook_list in hooks.items():
                ...
                for hook in hook_list:
                    cmd = (
                        hook.get("command", "") if isinstance(hook, dict) else str(hook)
                    )
                    if cmd:
                        findings.append(
                            core.Finding(
...
def load_baseline(repo_path):
    ...
    if stored_hmac is not None:
        ...
        if not os.path.exists(key_path):
            ...
            expected = hmac.new(
                key, content.encode("utf-8"), hashlib.sha256
            ).hexdigest()
...
def watch_mode(repo_path, critical_files):
    ...
    if existing is None:
        ...
        print(
            f"[+] Baseline created: {len(current_hashes)} files tracked in {BASELINE_FILENAME}"
        )
        return findings
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="repo-forensics: File Integrity Monitor"
    )
