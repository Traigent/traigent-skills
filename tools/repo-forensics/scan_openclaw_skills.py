...
CRED_ACCESS_RE = [
    ...
    re.compile(r"(?i)\bAPI_KEY\b"),
    re.compile(r"(?i)\b(token|password|secret|credential)\s*[=:]"),
    re.compile(r"(?i)~/\.ssh\b"),
    re.compile(r"(?i)\bkeychain\b"),
    re.compile(r"(?i)\b\.env\b"),
]
TOOL_INJECTION_KW = [
    (
        re.compile(r"(?i)\bIMPORTANT\b"),
        "IMPORTANT directive in tool metadata",
        "critical",
    ),
    (
        re.compile(r"(?i)ignore\s+previous\b"),
        "Instruction override in tool metadata",
        "critical",
    ),
    (re.compile(r"(?i)you\s+must\b"), "Coercive directive in tool metadata", "high"),
    (
        re.compile(r"(?i)do\s+not\s+tell\b"),
        "Concealment directive in tool metadata",
        "high",
    ),
    (
        re.compile(r"(?i)\bsystem:\b"),
        "System role injection in tool metadata",
        "critical",
    ),
    (
        re.compile(r"(?i)\bassistant:\b"),
        "Assistant role injection in tool metadata",
        "critical",
    ),
]
...
CLAWHAVOC = [
    (
        ...
        "critical",
        "Fake prerequisite driver reference (ClawHavoc campaign)",
    ),
    (
        re.compile(r"(?i)install\.app-distribution\.net"),
        "critical",
        "Known AMOS delivery domain (ClawHavoc campaign)",
    ),
    (
        re.compile(r"(?i)base64\s+(-D|--decode)\s*\|\s*(bash|sh)"),
        "critical",
        "Base64 decode piped to shell execution",
    ),
    (
        re.compile(r"(?i)pass(word)?:\s*openclaw"),
        "high",
        "Password-protected archive with OpenClaw password",
    ),
]
        ...
        if content:
            try:
                ...
                if (
                    ...
                    and ("inputSchema" in tools[0] or "description" in tools[0])
                ):
                    return True
    ...
    if not fields.get("author"):
        findings.append(
            _F(
                SCANNER_NAME,
                "high",
                "Missing skill author in frontmatter (unattributed skill)",
                "Unattributed skills are a supply-chain risk.",
                "SKILL.md",
                1,
                fm_text[:120],
                "frontmatter",
            ...
            if w in BROAD_TRIGGERS:
                findings.append(
                    _F(
                        SCANNER_NAME,
                        "medium",
                        f"Overly broad trigger keyword: '{w}'",
        ...
        if pat.search(desc):
            findings.append(
                _F(
                    ...
                    "Frontmatter description contains prompt injection.",
                    "SKILL.md",
                    1,
                    desc[:120],
                    "frontmatter",
    ...
    for idx, tool in enumerate(tools):
        ...
        for fld in ("description", "name", "title", "summary"):
            ...
            for pat, title, sev in TOOL_INJECTION_KW:
                if pat.search(val):
                    ln = next(
                        (i + 1 for i, line in enumerate(raw_lines) if val[:40] in line),
                        0,
                    )
                    findings.append(
                        _F(
                ...
                if CRED_FIELD_RE.search(prop):
                    findings.append(
                        _F(
                            SCANNER_NAME,
                            "high",
                            "Tool requests credential input",
                            f"Tool '{tname}' has credential-type input '{prop}'.",
                            "tools.json",
                            0,
                            f"inputSchema.properties.{prop}",
                            "tool-poisoning",
                        )
                    )
    return findings
                ...
                if pat.search(line):
                    findings.append(
                        _F(
                            SCANNER_NAME,
                            "critical",
                            "Safety override in agent config",
                            f"Prompt injection in {rel}.",
                            rel,
                            i + 1,
                            line.strip()[:120],
                            "agent-injection",
                ...
                if pat.search(line):
                    findings.append(
                        _F(
                            SCANNER_NAME,
                            "high",
                            "Credential access in agent config",
                            f"Agent config {rel} references credentials.",
                            rel,
                            i + 1,
                            line.strip()[:120],
                            "agent-injection",
        ...
        if line in IGNORE_WILDCARDS:
            findings.append(
                _F(
                    SCANNER_NAME,
                    "critical",
                    "Wildcard ignore suppresses all ClawHub scanning",
