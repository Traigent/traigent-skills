...
try:
    ...
    _iocs = _ioc.get_iocs()
    SANDWORM_KNOWN_IOC_PACKAGES = _iocs.get("malicious_npm", set()) | _iocs.get(
        "malicious_pypi", set()
    )
except ImportError:
...
def check_typosquatting(dependencies, popular_list):
    ...
    for dep in dependencies:
        ...
        for pop_lower, popular in popular_lower_map.items():
            if abs(len(dep_lower) - len(pop_lower)) > max(
                3, int(len(pop_lower) * 0.15)
            ):
        ...
        if dep.lower() in SANDWORM_KNOWN_IOC_PACKAGES:
            findings.append(
                core.Finding(
                    ...
                    file=rel_path,
                    line=0,
                    snippet=f"'{dep}' is a known malicious package",
                    category="known-ioc",
                )
            )
    return findings
        ...
        for suspect, target, score in typos:
            findings.append(
                core.Finding(
                    ...
                    file=rel_path,
                    line=0,
                    snippet=f"'{suspect}' might be typosquatting '{target}'",
...
def _collect_npm_deps_recursive(deps_dict, all_packages, depth=0):
    ...
    if (
        depth > 20 or len(all_packages) > 50000
...
def main():
    ...
    for file_path, rel_path in core.walk_repo(
        repo_path, ignore_patterns, skip_binary=True, skip_lockfiles=False
    ):
