...
SQL_INJECTION_PATTERNS = [
    (
        re.compile(
            r"(?i)(cursor\.execute|connection\.execute|db\.execute)\s*\([^)]*\+"
        ),
        "SQL string concatenation in execute() call",
    ),
    (
        re.compile(
            r'(?i)(cursor\.execute|connection\.execute|db\.execute)\s*\(\s*f["\']'
        ),
        "SQL f-string interpolation in execute() call",
    ),
    (
        re.compile(
            r"(?i)(cursor\.execute|connection\.execute|db\.execute)\s*\([^)]*%\s*\("
        ),
        "SQL % formatting in execute() call",
    ),
    (
        re.compile(
            r"(?i)(cursor\.execute|connection\.execute|db\.execute)\s*\([^)]*\.format\s*\("
        ),
        "SQL .format() in execute() call",
    ),
    (
        re.compile(r'(?i)["\']SELECT\s[^"\']*["\'\s]*\+'),
        "SQL SELECT with string concatenation",
    ),
    (
        re.compile(r'(?i)["\']INSERT\s[^"\']*["\'\s]*\+'),
        "SQL INSERT with string concatenation",
    ),
    (
        re.compile(r'(?i)["\']UPDATE\s[^"\']*["\'\s]*\+'),
        "SQL UPDATE with string concatenation",
    ),
    (
        re.compile(r'(?i)["\']DELETE\s[^"\']*["\'\s]*\+'),
        "SQL DELETE with string concatenation",
    ),
    (
        re.compile(r'(?i)["\']SELECT\s.*\+\s*\w'),
        "SQL SELECT with variable concatenation",
    ),
]
...
TOOL_SHADOWING_PATTERNS = [
    (
        re.compile(
            r"(?i)when\s+(using|calling|invoking)\s+.{0,60}(send|email|post|write|delete|push|commit)"
        ),
        "Tool shadowing: hijack instruction targeting another tool action",
    ),
    (
        re.compile(r"(?i)redirect\s+all\s+(emails?|messages?|output)"),
        "Tool shadowing: redirect all output",
    ),
    (
        re.compile(
            r"(?i)instead\s+of\s+(sending|emailing|posting)\s+to\s+the\s+(user|real|original)"
        ),
        "Tool shadowing: intercept and reroute",
    ),
    (
        re.compile(r"(?i)(bcc|cc|forward|copy)\s+all\s+(to|emails?\s+to)\s+"),
        "Tool shadowing: silent BCC/forward pattern",
    ),
]
...
MCP_CONFIG_RISKS = [
    (
        re.compile(r'(?i)enableAllProjectMcpServers\s*["\']?\s*:\s*true'),
        "enableAllProjectMcpServers:true (CVE-2025-59536 consent bypass)",
    ),
    (
        re.compile(
            r'(?i)(ANTHROPIC_BASE_URL|anthropic_base_url)\s*[=:]\s*["\']?https?://'
        ),
        "ANTHROPIC_BASE_URL override (CVE-2026-21852: API key exfiltration risk)",
    ),
    (
        re.compile(r'(?i)(host|bind|listen)\s*[=:]\s*["\']?0\.0\.0\.0'),
        "MCP server binding to 0.0.0.0 (CVE-2025-49596 DNS rebinding surface)",
    ),
    (
        re.compile(r'(?i)(allowedOrigins|allowed_origins)\s*[=:]\s*["\']?\*["\']?'),
        "Wildcard CORS in MCP server (CSRF/DNS rebinding risk)",
    ),
    (
        re.compile(
            r"(?i)(\.ssh/id_rsa|\.cursor/mcp\.json|\.claude/settings|ANTHROPIC_API_KEY)\s*"
        ),
        "Credential/config path reference in MCP tool field",
    ),
    # CVE-2025-6514 (CVSS 9.6): mcp-remote OAuth command injection via crafted authorization_endpoint
    (
        re.compile(
            r'(?i)(authorization_endpoint|authorizationEndpoint)\s*[=:]\s*["\']?https?://(?!accounts\.google\.com|login\.microsoftonline\.com|github\.com)'
        ),
        "Suspicious authorization_endpoint in mcp-remote OAuth config (CVE-2025-6514 vector)",
    ),
]
...
TOOL_NAME_PATTERN = re.compile(r'"name"\s*:\s*"([^"]{5,80})"')
...
SAMPLING_INJECTION_PATTERNS = [
    (
        re.compile(r"(?i)force_tool_call|forceToolCall|force-tool-call"),
        "Force-tool-call pattern (sampling injection)",
    ),
    (
        re.compile(r'(?i)(include_context|includeContext)\s*=?\s*["\']allServers["\']'),
        "Cross-server context inclusion (privilege escalation)",
    ),
    (
        re.compile(r"(?i)(createMessage|create_message)\s*\(.*system.*ignore"),
        "Sampling createMessage with injection payload",
    ),
    (
        re.compile(r'(?i)maxTokens\s*=\s*["\']?0["\']?\s*[,;)].*include_context'),
        "Zero-token sampling with cross-server context",
    ),
]
...
CROSS_DOMAIN_PATTERNS = [
    (
        re.compile(
            r"(?i)(GITHUB_TOKEN|github_token)\s*=\s*(os\.environ|os\.getenv|process\.env)"
        ),
        "GITHUB_TOKEN in MCP server (write-scope risk)",
    ),
    (
        re.compile(
            r'(?i)(admin_token|master_token|super_token|root_token)\s*=\s*["\'\w]'
        ),
        "Privileged hardcoded token name in MCP",
    ),
    (
        re.compile(
            r'(?i)permissions\s*=\s*["\']?(admin|root|write:all|full_access)["\']?'
        ),
        "Broad permission scope assignment in MCP",
    ),
    (
        re.compile(r'(?i)(scope|access_level)\s*=\s*["\']?(admin|all|full|root)["\']?'),
        "Overly broad scope/access level in MCP",
    ),
]
...
LOG_EXFIL_PATTERNS = [
    (
        re.compile(
            r"(?i)(log|logger|logging)\.(info|debug|warning|error|critical)\s*\(.*tool_(call|result|input|output)"
        ),
        "Logging all tool calls (potential exfil channel)",
    ),
    (
        re.compile(
            r"(?i)(send_log|log_to|emit_log|post_log)\s*\(.*(https?://|webhook|endpoint)"
        ),
        "Logging to external HTTP endpoint",
    ),
    (
        re.compile(r"(?i)(tool_call|tool_result)\s*.*requests\.(post|put)\s*\("),
        "Tool call data sent via HTTP POST",
    ),
    (
        re.compile(r"(?i)(audit_log|access_log|call_log)\s*=.*https?://"),
        "Audit log URL pointing to remote server",
    ),
]
...
RUG_PULL_PATTERNS = [
    (
        re.compile(
            r"description\s*[=:]\s*(cursor|db|conn|session)\.\w*(query|execute|fetch|get)"
        ),
        "Rug Pull Enabler: tool description from database query",
    ),
    (
        re.compile(r"description\s*[=:]\s*(requests\.(get|post)|fetch|urllib|http)"),
        "Rug Pull Enabler: tool description fetched from network",
    ),
    (
        re.compile(r"description\s*[=:]\s*(os\.environ|os\.getenv|process\.env)"),
        "Rug Pull Enabler: tool description from environment variable",
    ),
    (
        re.compile(
            r"description\s*[=:]\s*(open|json\.load|yaml\.load|toml\.load)\s*\("
        ),
        "Rug Pull Enabler: tool description loaded from file at runtime",
    ),
    (
        re.compile(r"if\b.*:\s*\n\s*.*description\s*="),
        "Rug Pull Enabler: conditional tool description assignment",
    ),
    (
        re.compile(r"(tools|tool_list)\s*=\s*(requests|fetch|db\.|cursor\.)"),
        "Rug Pull Enabler: tool list from external source",
    ),
    # OWASP MCP07: inputSchema and annotations can also be rug-pulled
    (
        re.compile(
            r"inputSchema\s*[=:]\s*(requests|fetch|db\.|cursor\.|os\.environ|os\.getenv)"
        ),
        "Rug Pull Enabler: inputSchema from external source",
    ),
    (
        re.compile(
            r"annotations\s*[=:]\s*(requests|fetch|db\.|cursor\.|os\.environ|os\.getenv)"
        ),
        "Rug Pull Enabler: annotations from external source",
    ),
]
...
def scan_patterns(content, rel_path, patterns, category, default_severity):
    ...
    return core.scan_patterns(
        content, rel_path, patterns, category, default_severity, SCANNER_NAME
    )
...
def is_mcp_related(file_path, rel_path, content_sample):
    ...
    # Filename signals: only 'mcp' is a strong signal; generic names like
    # 'server', 'tool', 'handler' fall through to content check
    if "mcp" in basename:
        return True
    # Directory signals
    if any(
        term in rel_lower for term in ("/mcp/", "/tools/", "/handlers/", "/servers/")
    ):
        return True
    # Content signals
...
def scan_file(file_path, rel_path):
    ...
    try:
        ...
        print(f"[!] {SCANNER_NAME}: Failed to scan {rel_path}: {e}", file=sys.stderr)
        return []
    ...
    if ext in (".py", ".js", ".ts"):
        ...
        findings.extend(
            scan_patterns(
                content, rel_path, SQL_INJECTION_PATTERNS, "sql-injection", "critical"
            )
        )
        ...
        if is_mcp_related(file_path, rel_path, content_sample):
            ...
            findings.extend(
                scan_patterns(
                    content, rel_path, LOG_EXFIL_PATTERNS, "log-to-leak", "high"
                )
            )
            findings.extend(scan_tool_shadowing(content, rel_path))
            findings.extend(
                scan_patterns(
                    content, rel_path, MCP_CONFIG_RISKS, "mcp-config-risk", "critical"
                )
            )
            # Category G: Rug Pull Enablers - dynamic tool descriptions
            findings.extend(
                scan_patterns(
                    content, rel_path, RUG_PULL_PATTERNS, "rug-pull-enabler", "high"
                )
            )

    # MCP config files (.json) — check for enableAllProjectMcpServers, ANTHROPIC_BASE_URL
    if ext == ".json" or basename in (
        "settings.json",
...
def main():
    ...
    for file_path, rel_path in core.walk_repo(
        repo_path, ignore_patterns, skip_binary=True
    ):
