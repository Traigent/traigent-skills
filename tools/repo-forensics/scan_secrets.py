...
PATTERNS = [
    ...
    {
        "name": "Private Key (RSA/PEM/EC/DSA/OPENSSH)",
        "severity": "critical",
        "regex": re.compile(
    ...
    },
    {
        ...
        "severity": "high",
        "regex": re.compile(
            r"(?i)(DefaultEndpointsProtocol|AccountKey|SharedAccessSignature)=[^\s;]+"
        ),
    },
    {
    ...
    # --- Medium: Generic Assignments ---
    {
        ...
        "severity": "medium",
        "regex": re.compile(
            r'(?i)(api_key|apikey|secret|token|password|auth_token|access_key|private_key)[^\'"\n=]{0,20}=\s*[\'"][0-9a-zA-Z\-_/+]{16,128}[\'"]'
        ),
    },
    {
...
def scan_file(file_path, rel_path):
    ...
    try:
        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
            ...
            lines = f.readlines()
            content = "".join(
                line for line in lines if len(line) <= core.MAX_LINE_LENGTH
            )
