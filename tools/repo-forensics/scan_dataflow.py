...
PYTHON_SOURCES = [
    ...
    (
        re.compile(r'(\w+)\s*=\s*getattr\s*\(\s*os\s*,\s*["\']environ["\']\)'),
        "getattr(os, 'environ') evasion",
    ),
    (
        re.compile(
            r"(\w+)\s*=\s*open\s*\([^)]*(?:\.env|\.ssh|\.aws|credentials|\.gnupg|\.config)"
        ),
        "Sensitive file read",
    ),
    (
        re.compile(
            r"(\w+)\s*=\s*pathlib\.Path\s*\([^)]*(?:\.env|\.ssh|\.aws|credentials)"
        ),
...
JS_SOURCES = [
    (
        re.compile(r"(?:const|let|var)\s+(\w+)\s*=\s*process\.env\b"),
        "process.env access",
    ),
    (
        re.compile(r"(?:const|let|var)\s+(\w+)\s*=\s*Object\.keys\s*\(\s*process\.env"),
        "process.env enumeration",
    ),
    (
        re.compile(
            r"(?:const|let|var)\s+(\w+)\s*=\s*(?:fs\.)?readFileSync\s*\([^)]*(?:\.env|\.ssh|\.aws|credential)"
        ),
        "Sensitive file read",
    ),
    (
        re.compile(r'(?:const|let|var)\s+(\w+)\s*=\s*require\s*\(["\']dotenv["\']\)'),
        "dotenv require",
    ),
    (
        re.compile(
            r"(?:const|let|var)\s+(\w+)\s*=\s*JSON\.parse\s*\(\s*(?:fs\.)?readFileSync"
        ),
        "JSON file parse",
    ),
]
    ...
    for file_path, rel_path in core.walk_repo(
        ...
        try:
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                content = "".join(
                    line for line in f if len(line) <= core.MAX_LINE_LENGTH
                )
        except Exception as e:
            print(
        ...
        for tmod in tainted_modules:
            if tmod in imported_modules:
                ...
                try:
                    ...
                    print(
                        f"[!] {SCANNER_NAME}: Failed to read {rel_path}: {e}",
                        file=sys.stderr,
                    )
                    continue
                ...
                for sink_pat in sinks:
                    ...
                    if m:
                        ...
                        snippet = (
                            content_lines[line_no - 1].strip()[:120]
