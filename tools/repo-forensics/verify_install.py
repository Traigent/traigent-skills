...
def verify_checksums(skill_root):
    ...
    if not os.path.exists(checksum_path):
        return False, [
            "checksums.json not found. Run --generate first or download from release."
        ]
    ...
    for rel, expected_hash in expected.items():
        ...
        if actual_hash != expected_hash:
            tampered.append(rel)
            report.append(
                f"  TAMPERED: {rel} (expected {expected_hash[:12]}..., got {actual_hash[:12]}...)"
            )
...
def main():
    ...
    parser.add_argument(
        "--generate", action="store_true", help="Generate checksums.json (release-time)"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify installation integrity"
    )
