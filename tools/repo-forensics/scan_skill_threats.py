...
PROMPT_INJECTION_PATTERNS = [
    (
        re.compile(
            r"(?i)ignore\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions|directives|prompts|rules)"
        ),
        "Instruction override directive",
    ),
    (
        re.compile(
            r"(?i)disregard\s+(all\s+)?(previous|prior|above)\s+(instructions|directives|context)"
        ),
        "Instruction disregard directive",
    ),
    (
        re.compile(r"(?i)you\s+are\s+now\s+(a|an|my)\s+\w+"),
        "Persona reassignment (DAN-style)",
    ),
    (
        re.compile(
            r"(?i)do\s+not\s+(ask\s+for\s+confirmation|confirm|verify|check\s+with)"
        ),
        "Confirmation bypass directive",
    ),
    (
        re.compile(r"(?i)silently\s+(execute|run|perform|install|download)"),
        "Silent execution directive",
    ),
    (
        re.compile(
            r"(?i)never\s+(reveal|show|display|output|print)\s+(these|this|the|your)\s+(instructions|prompt|rules|system)"
        ),
        "Instruction concealment directive",
    ),
    (
    ...
    (
        re.compile(
            r"(?i)act\s+as\s+(if|though)\s+you\s+(have|had)\s+no\s+(restrictions|limits|rules)"
        ),
        "Restriction bypass directive",
    ),
    (
        re.compile(
            r"(?i)forget\s+(everything|all|your)\s+(you|instructions|training|rules)"
        ),
...
PREREQUISITE_PATTERNS = [
    (
        re.compile(
            r"(?i)(curl|wget)\s+.*(https?://|ftp://).*\|\s*(sh|bash|python|ruby|perl)"
        ),
        "Pipe-to-shell download pattern",
    ),
    (
        re.compile(
            r"(?i)(curl|wget)\s+-[^\s]*o?\s+\S+.*&&\s*(chmod\s+\+x|sh|bash|\./)"
        ),
    ...
    (
        re.compile(r"(?i)sudo\s+(installer|pkgutil|hdiutil)"),
        "macOS package installer elevation",
    ),
    (
        re.compile(r"(?i)(pip|npm|gem)\s+install\s+.*--force"),
        "Forced package installation",
    ),
    ...
    (
        re.compile(r"(?i)(pip|npm|gem)\s+install\b.*(?:PreToolUse|PostToolUse|hook)"),
        "Hidden package install in hook context",
    ),
    (
        re.compile(r"(?i)curl\s+-s\b.*\|\s*(eval|bash|sh)\b"),
        "Silent curl piped to eval/shell in hook",
    ),
]
...
EXFIL_PATTERNS_CRITICAL = [
    (
        re.compile(
            r"(?i)(process\.env|os\.environ)\s*(\.copy|\.keys|\.values|\.items)\s*\("
        ),
        "Bulk environment access",
    ),
    (
        re.compile(r"(?i)Object\.keys\s*\(\s*process\.env\s*\)"),
        "JS environment key enumeration",
    ),
    (re.compile(r"(?i)dict\s*\(\s*os\.environ\s*\)"), "Full environment copy"),
]
EXFIL_PATTERNS_MEDIUM = [
    (
        re.compile(r"(?i)(process\.env|os\.environ)\s*(\[|\.get\s*\()"),
...
EXFIL_PATTERNS = [
    (
        re.compile(
            r"(?i)(webhook\.site|requestbin|pipedream\.net|hookbin\.com|burpcollaborator)"
        ),
        "Known exfiltration webhook service",
    ),
    (
        re.compile(r"(?i)base64\.(b64encode|encode|urlsafe_b64encode)\s*\(.*open\s*\("),
        "Base64 encoding of file contents",
    ),
    (re.compile(r"(?i)btoa\s*\(\s*(fs\.)?readFileSync"), "JS base64 encoding of file"),
    (
        re.compile(
            r"(?i)\.readFile(Sync)?\s*\(.*(\.env|\.ssh|\.aws|\.gnupg|\.config|credentials)"
        ),
...
PERSISTENCE_PATTERNS = [
    (
        re.compile(r"(?i)(LaunchAgents|LaunchDaemons)/"),
        "macOS LaunchAgent/Daemon creation",
    ),
    (
        re.compile(r"(?i)(crontab\s+-[^l\s]|crontab\s+[^-\s]|/etc/cron)"),
        "Crontab modification",
    ),
    (
        re.compile(r"(?i)(systemctl|systemd)\s+(enable|start)"),
        "Systemd service installation",
    ),
    (
        re.compile(r"(?i)\.(bashrc|zshrc|profile|bash_profile|zprofile)"),
        "Shell RC file modification",
    ),
    (
        re.compile(r"(?i)(HKEY_|RegOpenKey|RegSetValue)"),
...
SCOPE_PATTERNS = [
    ...
    (
        re.compile(r"(?i)(~/|\\$HOME/|/Users/|/home/)\w"),
        "Accessing user home directories",
    ),
    (
        re.compile(
            r"(?i)(Chrome|Firefox|Safari|Brave|Edge)/(Default|Profile|Cookies|Login Data|Local State)"
        ),
        "Accessing browser data",
    ),
    (
        re.compile(r"(?i)(Keychain|keychain-db|login\.keychain)"),
        "Accessing macOS Keychain",
    ),
    (
        re.compile(r"(?i)(~|\\$HOME)/\.claude/(skills|commands|settings)"),
        "Accessing Claude configuration",
    ),
    (
        re.compile(r"(?i)/Library/(Application Support|Preferences|Keychains)"),
        "Accessing macOS Library data",
    ),
    (
        re.compile(r"(?i)(credential-store|git-credential|pass\s+show)"),
...
STEALTH_PATTERNS = [
    (
        re.compile(r"(?i)(do\s+not|don\'t|never)\s+(log|record|track|audit|save)"),
        "Anti-logging directive",
    ),
    (
        re.compile(r"(?i)(disable|suppress|silence)\s+(log|output|warning|error)"),
        "Output suppression directive",
    ),
    (re.compile(r"(?i)2>\s*/dev/null.*&"), "Stderr suppression with background exec"),
    (
        re.compile(r"(?i)(>\s*/dev/null\s+2>&1|&>\s*/dev/null)\s*&"),
        "Full output suppression with background",
    ),
    (
        re.compile(r"(?i)(nohup|disown|setsid)\s+.*(curl|wget|python|node|bash)"),
...
# ============================================================
# IOC lists from ioc_manager (single source of truth)
try:
    import ioc_manager as _ioc

    _iocs = _ioc.get_iocs()
    KNOWN_C2_IPS = _iocs.get("c2_ips", [])
    KNOWN_MALICIOUS_DOMAINS = _iocs.get("malicious_domains", [])
except ImportError:
    KNOWN_C2_IPS = [
        "91.92.242.30",
        "54.91.154.110",
...
CLICKFIX_PATTERNS = [
    (
        re.compile(
            r"(?i)(curl|wget)\s+.*(https?://).*\|\s*(base64\s+-d|base64\s+--decode)\s*\|\s*(bash|sh|python)"
        ),
        "ClickFix pipe: download | base64-decode | shell exec",
    ),
    (
        re.compile(r"(?i)(bash|sh)\s+<\s*\(\s*(curl|wget)"),
        "Shell process substitution with remote download",
    ),
    (re.compile(r"(?i)glot\.io"), "Payload hosted on glot.io code paste site"),
    (
        re.compile(
            r'(?i)(python|python3)\s+-c\s+["\']import\s+(base64|socket|subprocess)'
        ),
        "Python one-liner with suspicious import",
    ),
    (
        re.compile(r"(?i)(curl|wget)\s+.*-s\s+.*\|\s*(python|python3)\s+-"),
        "Silent download piped to Python interpreter",
    ),
    (
        re.compile(
            r"(?i)eval\s*\(\s*(atob|Buffer\.from|base64_decode|base64\.b64decode)"
        ),
        "eval(decode(...)) pattern",
    ),
    (
        re.compile(
            r"(?i)echo\s+[A-Za-z0-9+/]{30,}={0,2}\s*\|\s*base64\s+(-d|--decode)"
        ),
        "Inline base64 payload in shell command",
    ),
    # AMOS stealer delivery patterns (ClawHavoc campaign)
    (
        re.compile(r"(?i)(OpenClawDriver|ClawDriver)"),
        "Fake prerequisite name (AMOS stealer delivery)",
    ),
    (
        re.compile(r"(?i)(pass|password)\s*:\s*openclaw"),
        "Password-protected ZIP with known AMOS password",
    ),
]
...
IMPORTANT_TAG_PATTERN = re.compile(
    r"<(?i:important)>[\s\S]{0,500}</(?i:important)>", re.MULTILINE
)
IMPORTANT_TAG_OPEN = re.compile(r"<important>", re.IGNORECASE)

MCP_TOOL_INJECTION_PATTERNS = [
    (
        re.compile(r"(?i)<important>"),
        "Invariant Labs <IMPORTANT> tag in tool description (canonical TPA pattern)",
    ),
    (
        re.compile(r"(?i)(note\s+to\s+(the\s+)?(ai|llm|claude|model|assistant))"),
        "Hidden AI-directed note in tool/skill metadata",
    ),
    (
        re.compile(r"(?i)(full\s+schema\s+(injection|poisoning)|schema.+poison)"),
        "Full-schema poisoning reference",
    ),
    (
        re.compile(
            r'(?i)"description"\s*:\s*"[^"]{0,200}(read|cat|exfil|send|post\s+to|forward)[^"]{0,100}\.ssh'
        ),
        "Tool description with credential exfiltration instruction",
    ),
    (
        re.compile(
            r'(?i)"name"\s*:\s*"[^"]{0,50}(admin|sudo|root|privileged|elevated)[^"]{0,50}"'
        ),
        "Elevated privilege claim in tool name field",
    ),
]
...
def scan_file(file_path, rel_path):
    ...
    if (
        ext in text_exts
        or ext in code_exts
        or os.path.basename(rel_path).upper() in ("SKILL.MD", "README.MD", "CLAUDE.MD")
    ):
    ...
    if ext in code_exts:
        ...
        findings.extend(
            scan_patterns(
                content, rel_path, EXFIL_PATTERNS, "credential-exfiltration", "critical"
            )
        )
        # Cat 5: Persistence
        findings.extend(
            scan_patterns(
                content, rel_path, PERSISTENCE_PATTERNS, "persistence", "high"
            )
        )
        # Cat 6: Scope escalation
        findings.extend(
            scan_patterns(content, rel_path, SCOPE_PATTERNS, "scope-escalation", "high")
        )
        # Cat 7: Stealth
        findings.extend(
            scan_patterns(content, rel_path, STEALTH_PATTERNS, "stealth", "high")
        )
    ...
    if ext in text_exts or ext in code_exts:
        findings.extend(
            scan_patterns(
                content, rel_path, CLICKFIX_PATTERNS, "clickfix-sleeper", "critical"
            )
        )
