...
def get_file_type(filepath):
    try:
        ...
        print(f"[!] {SCANNER_NAME}: Failed to read {filepath}: {e}", file=sys.stderr)
        return None
...
def main():
    ...
    for filepath, rel_path in core.walk_repo(
        repo_path, ignore_patterns, skip_binary=False
    ):
        ...
        if os.name == "posix":
            if os.access(filepath, os.X_OK) and ext in {
                ".png",
                ".jpg",
                ".txt",
                ".md",
