#!/usr/bin/env python3
"""Propagate the canonical run-cost card into every skill that points at it.

Source of truth: docs/shared/run-cost-and-limits.v1.md
Generated copies: skills/<name>/references/run-cost-and-limits.md
...
This does not create several sources of truth. The doc is authored in exactly one place;
the per-skill files are generated artifacts, byte-identical to the source, refreshed by
this tool and pinned by tests/contract/test_run_cost_reference.py. Editing a generated
copy is a defect the contract suite fails on. This mirrors
sync_economics_reference.py and sync_interaction_policy.py for the same reason.

Target skills are discovered, not hardcoded: any SKILL.md carrying the run-cost pointer
marker gets the reference.

Usage:
  python tools/contract/sync_run_cost_reference.py          # write mode (default)
  python tools/contract/sync_run_cost_reference.py --check  # exit 1 if any copy is
...
# The pointer every run-cost SKILL.md carries to its local copy. Presence of this marker is
# what marks a skill as a carrier of the card.
...
def canonical_text(root: Path) -> str:
    ...
    if not source.is_file():
        raise SystemExit(f"Canonical run-cost reference not found: {CANONICAL_RELPATH}")
    return source.read_text(encoding="utf-8")


def run_cost_skill_dirs(root: Path) -> list[Path]:
    """Skill dirs whose SKILL.md points at the local run-cost reference."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sync the canonical run-cost card into every skill that points at it."
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    targets = run_cost_skill_dirs(root)
    ...
    if args.check:
        if changed or orphans:
            ...
            return 1
        print(f"All {len(targets)} run-cost skill(s) carry the current reference.")
        return 0
    ...
    if changed:
        ...
        print(f"All {len(targets)} run-cost skill(s) already up to date.")
    print(
